...
async function beforeUpload(file: any) {
  ...
  fileList.value = [compressedFile as any]
...
function onFinish() {
  ...
  if (!fileList.value?.[0] && formData.value.articleCover) {
    ...
    const coverFile = fileList.value?.[0] as any
    const articleCover = new FormData()
    articleCover.append('articleCover', coverFile, coverFile?.name || 'cover')
  ...
  const res = await Promise.all(
    files.map(async (file: any) => {
      ...
      return new Promise((rev, rej) => {
        ...
        form.append('articleImage', compressedFile, compressedFile.name || 'image')
  ...
  <layout :is-list="false">
    ...
    <template #table-content>
      <div style="height: 80vh;width: 100%">
        <MdEditor v-model="formData.articleContent" :theme="(mode as any) as 'light' | 'dark'" style="height: 80vh" :toolbars="toolbars as []" @onUploadImg="onUploadArticleImg" />
      </div>
    </template>
  </layout>
</template>
