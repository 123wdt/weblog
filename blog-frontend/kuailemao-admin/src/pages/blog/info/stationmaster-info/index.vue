...
const formData: any = reactive(props.info as object)
...
if(formData.webmasterAvatar && formData.webmasterProfileBackground){
  imageAvatarUrl.value = formData.webmasterAvatar
  const myUrl = new URL(formData.webmasterProfileBackground as string);
  const fileName = myUrl.pathname.split('/').pop() || 'background';
  backFileList.value = [{
    uid: fileName,
...
async function beforeUploadAvatar(file: any) {
  loading.value = true
  ...
  webmasterAvatar.append('avatar', compressedFile, (compressedFile as File).name || 'avatar')
...
async function beforeUploadAckgroundImag(file: any) {
  const isJpgOrPng = file.type === 'image/jpeg' || file.type === 'image/png' || file.type === 'image/webp'
  if (!isJpgOrPng){
    message.error('文件格式必须是jpg或png或webp')
    return
  ...
  const webmasterAvatar = new FormData()
  webmasterAvatar.append('background', compressedFile, (compressedFile as File).name || 'background')
  uploadAckgroundImage(webmasterAvatar).then((res) => {
    if (res.code === 200) {
      backFileList.value = [{
        uid: new URL(res.data).pathname.split('/').pop() || 'background',
        thumbUrl: res.data,
