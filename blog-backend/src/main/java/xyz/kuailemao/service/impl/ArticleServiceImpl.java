...
public class ArticleServiceImpl extends ServiceImpl<ArticleMapper, Article> implements ArticleService {
    ...
    public ResponseResult<Void> deleteArticleCover(String articleCoverUrl) {
        try {
            // 提取七牛云文件 key（URL 中去掉域名前缀部分）
            String key = articleCoverUrl.substring(articleCoverUrl.indexOf(qiniuDomain) + qiniuDomain.length() + 1);
            fileUploadUtils.deleteFiles(List.of(key));
            return ResponseResult.success();
        } catch (Exception e) {
